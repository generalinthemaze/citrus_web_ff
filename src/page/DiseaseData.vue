    ...
    <div>
        <Card :bordered="false" style="min-height: 650px">
            <Row slot="title" type="flex" justify="end" :gutter=16>
                <i-col span="6">
                    <Input search enter-button placeholder="搜索病害"
                           @on-search="search"/>
                </i-col>
                <i-col>
            ...
            <Table border :loading="tableLoading" :columns="columns" :data="data" ref="diseaseTable"></Table>
        ...
        <Modal v-model="editModalShow"
            ...
            <Form :model="editItemData" :label-width="80">
                <Row type="flex" justify="start">
                    <i-col span="12">
                        <FormItem label="病害编号">
                            <Input v-model="editItemData.diseaseId" :disabled="isEdit"/>
                        </FormItem>
                    </i-col>
                    <i-col span="12">
                        <FormItem label="中文名称">
                ...
                <FormItem label="病原">
                    <Input v-model="editItemData.pathogeny"/>
                </FormItem>
                <FormItem label="发生规律">
...
<script>
    import {update,del,add,search} from "../api/disease";
    ...
    export default {
        ...
        methods:{
            ...
            exportData(){
                this.$refs.diseaseTable.exportCsv({
                    filename: '柑橘病害_'+new Date().toLocaleString()
                })
            },
            ...
            handleImgBeforeUpload(file){
                this.$Loading.start();
                uploadFile('disease_image/'+this.editItemData.diseaseId+'/'+file.name,file).then(result=>{
